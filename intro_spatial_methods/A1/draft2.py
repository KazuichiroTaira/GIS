...
def create_orthophoto_fit_lazer(laser_x_min, laser_x_max, laser_y_min, laser_y_max, grid_size):

    len_x = laser_x_max - laser_x_min
    x_points = len_x / grid_size
    x_points_as_row = math.ceil(x_points)

    len_y = laser_y_max - laser_y_min
...
def main():
    ...
    laser_x_min = laser_x.min()
    laser_x_max = laser_x.max()
    laser_y_min = laser_y.min()
    laser_y_max = laser_y.max()

    grid_size = 0.2

    x_points_as_row, y_points_as_columns = create_orthophoto_fit_lazer(laser_x_min, laser_x_max, laser_y_min,
                                                                       laser_y_max, grid_size)

    print(f"row {x_points_as_row}, columns {y_points_as_columns}")
    ...
    pixels_rows, pixels_columns = x_points_as_row, y_points_as_columns  # use (1, 3) or orthophoto.shape
